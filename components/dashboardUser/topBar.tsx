...
const Topbar = () => {
  return (
    <motion.div
      dir="rtl"
      className=" bg-white/10 shadow-md p-4 mx-4 mt-32 flex justify-between rounded-2xl items-center my-5"
      initial={{ y: -300 }}
      animate={{ y: 0 }}
      transition={{ duration: 0.5 }}
    >
      <h1 className="text-xs md:text-lg ml-3 font-bold text-white">
      ...
      <div>
        <div className="relative" id="input">
          <input
            placeholder="جستحو..."
            className="block mr-auto w-[230px] text-sm h-[50px] p-4 text-slate-900 bg-white rounded-[8px] border border-slate-200 appearance-none focus:border-transparent focus:outline focus:outline-2 focus:outline-primary focus:ring-0 hover:border-brand-500-secondary peer invalid:border-error-500 invalid:focus:border-error-500 overflow-ellipsis overflow-hidden text-nowrap pr-[48px]"
            id="floating_outlined"
            type="text"
          />
          <label
            className="peer-placeholder-shown:-z-10 peer-focus:z-10 absolute text-[14px] leading-[150%] text-primary peer-focus:text-primary peer-invalid:text-error-500 focus:invalid:text-error-500 duration-300 transform -translate-y-[1.2rem] scale-75 top-2 z-10 origin-[0] bg-yellow-400 p-1 data-[disabled]:bg-gray-50-background- px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-[1.2rem] rtl:peer-focus:translate-x-1/4 rounded-2xl rtl:peer-focus:left-auto start-1"
