...
const Page = () => {
  const blogData = {
    ...
    url: "/Text-Classifiers-in-Machine-Learning-A-Practical-Guide-in-persian",
    images: ["/assets/images/text_classification_guide.svg"],
    sections: [
      {
        heading: "طبقه‌بندی‌کننده متن یا (text classifier) چیست؟",
        content:
      ...
      {
        heading: "مدل چیست؟ تفاوت مدل و طبقه‌بندی‌کننده متن",
        content:
          "مدل در یادگیری ماشین به سیستمی اطلاق می‌شود که بر اساس داده‌های ورودی، تحلیل‌ها و پیش‌بینی‌هایی انجام می‌دهد. در حالی که طبقه‌بندی‌کننده متن یک نوع خاص از مدل است که وظیفه برچسب‌گذاری داده‌های متنی را بر عهده دارد. به عنوان مثال، می‌توان از شبکه‌های عصبی (Neural Networks) برای انجام این عمل لیبل‌گذاری استفاده کرد.",
      },
      {
        heading: "نمونه تمرینی یا Training Sample",
        content:
          "نمونه آموزشی در یادگیری ماشین به واحدی از داده‌ها اشاره دارد که برای آموزش مدل استفاده می‌شود. این نمونه می‌تواند شامل ورودی (x) و خروجی (y) باشد، مانند ویژگی‌های یک خانه و قیمت آن در یادگیری نظارت‌شده (Supervised Learning). در مسائل بدون نظارت (Unsupervised Learning)، نمونه آموزشی معمولاً شامل داده‌های بدون برچسب است که مدل از آن‌ها برای کشف الگوها یا ساختارها استفاده می‌کند.",
      },
      {
        heading: "تابع هدف یا Target Function",
        content:
          "تابع هدف، ایده‌آل یا تابع حقیقی است که توصیف‌کننده دقیق رابطه بین داده‌هاست، اگرچه معمولاً ناشناخته است. مدل یادگیری ماشین تلاش می‌کند تا این تابع را از طریق داده‌های آموزشی تقریب بزند. در یادگیری نظارت‌شده، این تابع به صورت f(x) = y تعریف می‌شود.",
      },
      {
        heading: "فرضیه یا Hypothesis در یادگیری ماشین",
        content:
          "فرضیه در یادگیری ماشین تابعی پیشنهادی است که مدل بر اساس داده‌های آموزشی یاد می‌گیرد و به عنوان تقریب تابع هدف عمل می‌کند. این تابع مجموعه‌ای از قواعد یا مرزهای تصمیم‌گیری است که مدل برای پیش‌بینی‌ها از آن استفاده می‌کند.",
      },
      {
        heading: "الگوریتم یادگیری یا Learning Algorithm",
        content:
          "الگوریتم یادگیری در یادگیری ماشین مجموعه‌ای از دستورالعمل‌هاست که با استفاده از داده‌های آموزشی تلاش می‌کند تا تابع هدف را پیدا کرده یا تقریب بزند. این الگوریتم از دو بخش اصلی تشکیل شده است: تابع خطا (Loss Function) که مشخص می‌کند پیش‌بینی مدل تا چه حد با نتایج واقعی فاصله دارد، و روش بهینه‌سازی (Optimization Technique) که پارامترهای مدل را طوری تنظیم می‌کند که این فاصله (خطا) کمتر شود.",
      },
      {
        heading: "انتخاب الگوریتم مناسب",
        content:
          "در پروژه‌های طبقه‌بندی متن، انتخاب الگوریتم مناسب بسیار حائز اهمیت است. برخی از الگوریتم‌های استاندارد عبارتند از: رگرسیون لجستیک (Logistic Regression)، نایو بیز (Naïve Bayes)، گرادیان نزولی تصادفی (Stochastic Gradient Descent)، الگوریتم K-نزدیک‌ترین همسایه (K-Nearest Neighbors)، درخت تصمیم (Decision Tree)، جنگل تصادفی (Random Forest) و ماشین بردار پشتیبان (Support Vector Machine).",
      },
      {
        heading: "ارزیابی عملکرد مدل طبقه‌بندی متن",
        content:
          "برای ارزیابی عملکرد مدل‌های طبقه‌بندی متن، از معیارهایی مانند دقت (Accuracy)، یادآوری (Recall)، دقت پیش‌بینی (Precision) و امتیاز F1 استفاده می‌شود. این معیارها به ما کمک می‌کنند تا کارایی مدل را بسنجیم و در صورت نیاز، بهبود دهیم.",
      },
      {
        heading: "جمع‌بندی و خلاصه مطالب",
        content:
          "طبقه‌بندی متن یکی از کاربردهای مهم یادگیری ماشین است که به ما امکان می‌دهد داده‌های متنی را به صورت خودکار برچسب‌گذاری و تحلیل کنیم. با انتخاب الگوریتم مناسب و ارزیابی دقیق مدل، می‌توان به نتایج قابل اعتمادی دست یافت.",
      },
    ],
  };
  ...
  useEffect(() => {
    document.title = "راهنمای جامع طبقه‌بندی متون";
